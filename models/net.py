...
def highPassFilter(x, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")):
...
class CC_HEAD(nn.Module):
    def __init__(self, in_out_channels, dropout_p=0.5, act='sigmoid'):
        ...
        self.base = BasicBlock(in_out_channels, in_out_channels, dilation=2)

        self.fc = nn.Sequential(nn.Conv2d(in_out_channels, 256, 1, stride=1, padding=0),
                                nn.ReLU(inplace=True),
...
def get_feature_net(num_layers, down_ratio=4):
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.C1 = get_CC_head(in_channels=64)
        initialize_weights(self.modules())
        self.G = get_feature_net(34, down_ratio=4)

    def forward(self, img):
        feature = self.G(img)

        output, logit = self.C1(feature)
